...
mandatory **2-of-2 guardian taproot** deposit address. The ref-app must (a) exercise the _full_
...
| 3   | **Deposit address**                       | `generateDepositAddress({ suiAddress })` — labelled as 2-of-2 `tr(NUMS, multi_a(2, guardian, mpc-child))`; **graceful `HashiConfigError`** state when guardian unprovisioned. Optional `bitcoin.lookupAllVouts` when `VITE_BTC_RPC_URL` set | 4 Deposit Address Derivation                            |
